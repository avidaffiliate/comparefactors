...
def map_outcome_type(outcome_type: str, outcome: str, has_pfeo: bool) -> dict:
    ...
    if outcome_type == 'Dismissed':
        ...
        if 'PFEO' in outcome and 'Non-Compliant' in outcome:
            ...
            # Generic - need to check has_pfeo
            if has_pfeo:
                result['ai_outcome'] = 'Upheld'
                result['pfeo_issued'] = True
            else:
                result['ai_outcome'] = 'Upheld'  # Assume upheld if decision issued
    ...
    # Override with has_pfeo if set
    if has_pfeo and not result['pfeo_issued'] and not result['pfeo_proposed']:
        result['pfeo_issued'] = True
    
    # Can't have PFEO and Dismissed - PFEO means factor was found at fault
